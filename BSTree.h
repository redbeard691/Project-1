...
public:
	...
	*/
	bool insert(Value value, KeyComparable key) {
		this.insert(0, value, key)
	}
	// Helper function for inserting recursively
	// TODO: this can go out of bounds.
	bool insert(int index, Value value, KeyComparable key) 
	{
		...
		{
			// check left
			this.insert(index * 2, value, key);
		}
		else root[index] < value) 
		{
			// check right
			this.insert(index * 2 + 1, value, key);
		}
		else 
		{
			return false;
